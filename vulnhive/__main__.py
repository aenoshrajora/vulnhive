...
class vulnhiveManager:
    def __init__(self, options: Namespace, server_args: dict[str, str | int]):
        ...
        self.vulnhive: list[tuple[Any, str, bool]] = []

    def main(self):
        logger.info("For updates, check https://github.com/qeeqbox/vulnhive")
    ...
    def _set_up_vulnhive(self):  # noqa: C901
        register(_exit_handler)
        if self.options.termination_strategy == "input":
            logger.info("Use [Enter] to exit or python3 -m vulnhive --kill")
        ...
        running_vulnhive = {"good": [], "bad": []}
        if len(self.vulnhive) > 0:
            for _, server_name, status in self.vulnhive:
                if not status:
                    running_vulnhive["bad"].append(server_name)
                else:
                    running_vulnhive["good"].append(server_name)

            if len(running_vulnhive["good"]) > 0:
                running_servers = ", ".join(running_vulnhive["good"])
                logger.info(f"servers {running_servers} running...")

            if len(running_vulnhive["bad"]) > 0:
                not_running_servers = ", ".join(running_vulnhive["bad"])
                logger.warning(f"servers {not_running_servers} not running...")

            if len(running_vulnhive["bad"]) == 0:
                logger.info("Everything looks good!")

            if len(running_vulnhive["good"]) > 0 and not self.options.test:
    ...
    def _start_all_servers(self):
        try:
            ...
            logger.exception(f"Starting vulnhive failed: {error}")
    ...
    def _start_server(self, service: str, auto: bool | None = None):
        ...
        if not self.options.test:
            ...
            server.kill_server()
            status = False
        self.vulnhive.append((server, service, status))

    def _stop_servers(self):
        logger.info("[x] Stopping servers...")
        for server, name, _ in self.vulnhive:
            try:
    ...
    def _start_chameleon_mode(self):  # noqa: C901,PLR0912
        ...
        vulnhive = self.config_data["vulnhive"]
        if isinstance(vulnhive, dict):
            logger.info("[x] Parsing honeypot [hard]")
            for honeypot in vulnhive:
                self._start_server(honeypot)
        elif isinstance(vulnhive, str):
            logger.info("[x] Parsing honeypot [easy]")
            if ":" in vulnhive:
                logger.error(
                    "[!] You cannot bind ports with [:] in this mode, "
                    "use the vulnhive dict instead"
                )
                sys.exit(1)
            for server in vulnhive.split(","):
                self._start_server(server)
        else:
            logger.error(f"[!] Unable to parse vulnhive from config: {vulnhive}")
            sys.exit(1)
    ...
    def _setup_logging(self) -> logging.Logger:
        uuid = f"vulnhivelogger_main_{str(uuid4())[:8]}"
    ...
    def _start_sniffer(self, sniffer_filter, sniffer_interface):
        ...
        sleep(0.1)
        self.vulnhive.append((sniffer, "sniffer", sniffer.server_is_alive()))
...
def _parse_args() -> tuple[Namespace, dict[str, str | int]]:
    arg_parser = _ArgumentParser(
        description=(
            "Qeeqbox/vulnhive customizable vulnhive for monitoring network traffic, bots "
    ...
    arg_parser_setup.add_argument(
        "--list", action="store_true", help="list all available vulnhive"
    )
    arg_parser_setup.add_argument("--kill", action="store_true", help="kill all vulnhive")
    arg_parser_setup.add_argument("--verbose", action="store_true", help="Print error msgs")
    arg_parser_optional = arg_parser.add_argument_group("vulnhive options")
    ...
    arg_parser_optional.add_argument(
        "--config",
        help="Use a config file for vulnhive settings",
        metavar="",
        default="",
    )
    arg_parser_optional.add_argument(
...
def main_logic():
    ...
    manager = vulnhiveManager(argv, server_args)
    manager.main()
