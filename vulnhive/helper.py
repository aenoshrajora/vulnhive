...
def set_up_error_logging():
    _logger = logging.getLogger("vulnhive.error")
...
def set_local_vars(self, config: str | None = None):
    ...
    try:
        ...
        vulnhive = config_data.get("vulnhive", [])
        honeypot = self.__class__.__name__[1:-6].lower()
        if honeypot and honeypot in vulnhive:
            for attr, value in vulnhive[honeypot].items():
...
def _serialize_message(  # noqa: C901
    ...
    try:
        if custom_filter:
            filters = custom_filter.get("vulnhive", {})
...
def _parse_record(record: LogRecord, custom_filter: dict, type_: str) -> LogRecord | None:
    ...
    with suppress(Exception):
        if type_ == "file":
            if custom_filter:
                options = custom_filter.get("vulnhive", {}).get("options", [])
...
def setup_logger(name: str, temp_name: str, config: str | None, drop: bool = False):
    ...
    if "file" in logs:
        ...
        server_config = config_data.get("vulnhive", {}).get(server, {})
    ...
    def emit(self, record: LogRecord):  # noqa: C901,PLR0912
        try:
            ...
            if (
                ...
                and "vulnhive" in self.custom_filter
                and "remove_errors" in self.custom_filter["vulnhive"].get("options", [])
            ):
                return
